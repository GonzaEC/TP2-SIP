 ...
 * WebDriver real.
 *
 * <p>HIT 8 extiende HIT 6 con: paginación (hasta 3 páginas / 30 resultados), estadísticas de precio
...
class MercadoLibreScraperTest {
  ...
  // ── 1. Cantidad por página (extraerDatos usa CANT_POR_PAGINA) ────────────

  @Test
  void extraerDatosConMasDePortPaginaContenedoresRetornaExactamentePorPagina() {
    ...
    assertEquals(
        MercadoLibreScraper.CANT_POR_PAGINA,
        resultados.size(),
        "extraerDatos debe respetar CANT_POR_PAGINA por página");
  }
  ...
  @Test
  void extraerDatosConPaginacionSinSiguienteRetornaResultadosDePrimeraPagina() {
    List<WebElement> paginaUno = buildContainerList(10);
    when(driver.findElements(Selectors.CONTENEDOR_RESULTADOS)).thenReturn(paginaUno);
    // No hay botón siguiente → NoSuchElementException
    when(driver.findElement(Selectors.BOTON_SIGUIENTE_PAGINA))
        .thenThrow(new NoSuchElementException("no next"));

    List<ProductResult> resultados =
        MercadoLibreScraper.extraerDatosConPaginacion(driver, wait, "test");

    assertEquals(10, resultados.size());
  }

  @Test
  void extraerDatosConPaginacionNuncaExcedeElTotalObjetivo() {
    List<WebElement> contenedores = buildContainerList(MercadoLibreScraper.CANT_POR_PAGINA);
    ...
    assertTrue(resultados.size() <= MercadoLibreScraper.CANT_RESULTADOS);
  }

  @Test
  void irSiguientePaginaRetornaFalseSiNoHayBoton() {
    WebDriver jsDriver =
        mock(WebDriver.class, withSettings().extraInterfaces(JavascriptExecutor.class));
    when(jsDriver.findElement(Selectors.BOTON_SIGUIENTE_PAGINA))
        .thenThrow(new NoSuchElementException("no next"));

    assertFalse(MercadoLibreScraper.irSiguientePagina(jsDriver, wait));
  }
